// Code generated by mockery v2.13.1. DO NOT EDIT.

package mocks

import (
	jwt "github.com/gbrlsnchs/jwt"
	mock "github.com/stretchr/testify/mock"
)

// Tokener is an autogenerated mock type for the Tokener type
type Tokener struct {
	mock.Mock
}

// Create provides a mock function with given fields: userID, userType
func (_m *Tokener) Create(userID uint, userType uint) (string, error) {
	ret := _m.Called(userID, userType)

	var r0 string
	if rf, ok := ret.Get(0).(func(uint, uint) string); ok {
		r0 = rf(userID, userType)
	} else {
		r0 = ret.Get(0).(string)
	}

	var r1 error
	if rf, ok := ret.Get(1).(func(uint, uint) error); ok {
		r1 = rf(userID, userType)
	} else {
		r1 = ret.Error(1)
	}

	return r0, r1
}

// CreateRefresh provides a mock function with given fields: userID, userType, fgp
func (_m *Tokener) CreateRefresh(userID uint, userType uint, fgp *string) (string, error) {
	ret := _m.Called(userID, userType, fgp)

	var r0 string
	if rf, ok := ret.Get(0).(func(uint, uint, *string) string); ok {
		r0 = rf(userID, userType, fgp)
	} else {
		r0 = ret.Get(0).(string)
	}

	var r1 error
	if rf, ok := ret.Get(1).(func(uint, uint, *string) error); ok {
		r1 = rf(userID, userType, fgp)
	} else {
		r1 = ret.Error(1)
	}

	return r0, r1
}

// Validate provides a mock function with given fields: _a0
func (_m *Tokener) Validate(_a0 *string) (*jwt.JWT, error) {
	ret := _m.Called(_a0)

	var r0 *jwt.JWT
	if rf, ok := ret.Get(0).(func(*string) *jwt.JWT); ok {
		r0 = rf(_a0)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(*jwt.JWT)
		}
	}

	var r1 error
	if rf, ok := ret.Get(1).(func(*string) error); ok {
		r1 = rf(_a0)
	} else {
		r1 = ret.Error(1)
	}

	return r0, r1
}

type mockConstructorTestingTNewTokener interface {
	mock.TestingT
	Cleanup(func())
}

// NewTokener creates a new instance of Tokener. It also registers a testing interface on the mock and a cleanup function to assert the mocks expectations.
func NewTokener(t mockConstructorTestingTNewTokener) *Tokener {
	mock := &Tokener{}
	mock.Mock.Test(t)

	t.Cleanup(func() { mock.AssertExpectations(t) })

	return mock
}
